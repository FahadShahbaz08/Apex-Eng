...
export function ItemForm({ close, defaultType = "Finished Good", item = null }) {
  ...
  return <Modal title={item ? `Edit ${item.name}` : rawOnly ? "Add raw material" : "Add item or product"} eyebrow={rawOnly ? "RAW MATERIAL MASTER" : "ITEM MASTER"} close={close} wide><form onSubmit={submit}>
    <div className="form-grid three">
      ...
      {rawOnly && <><Field label="Default overall labour cost (optional)"><input name="defaultLabourCharge" type="number" min="0" step="0.01" defaultValue={item?.defaultLabourCharge || 0} /></Field><Field label="Default labour supplier account"><SearchableSelect name="labourSupplierId" defaultValue={labourSupplierId} required searchPlaceholder="Search suppliers…"><option value="">Select supplier</option>{labourSuppliers.map(party => <option key={party.id} value={party.id}>{party.name}</option>)}</SearchableSelect></Field></>}
    ...
    <p className="form-note">{rawOnly ? "The optional overall labour cost and supplier are copied into each new raw-material sale. The charge is applied once and is never multiplied by quantity; it can be changed on that invoice without changing older invoices. Abu Ki Mazduri is selected automatically when that supplier exists." : type === "Finished Good" ? "Labour and laser rates are copied into each new invoice. Later product edits never change old invoices. Abu Ki Mazduri is selected automatically when that supplier exists." : item ? "Use a stock adjustment to change on-hand quantity; editing keeps the movement ledger intact." : "The SKU is generated from the item type and the next available sequence number."}</p>
