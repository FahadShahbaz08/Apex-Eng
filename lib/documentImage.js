...
const downloadCanvas = (canvas, filename, delay = 0) => new Promise((resolve, reject) => {
  canvas.toBlob(blob => {
...
const loadExportLogo = () => {
  if (!exportLogoPromise) exportLogoPromise = new Promise(resolve => {
...
export async function downloadTableImage({ company, title, subtitle, headers, rows, filename, summary = [], summaryLeft = [], meta = [], featuredMetaIndex = -1, footerMessage = "Thank you for your business" }) {
  const exportLogo = await loadExportLogo();
  const scale = 2;
  const width = 1600;
  const margin = 54;
  const usable = width - margin * 2;
  const headerHeight = featuredMetaIndex >= 0 ? 318 : 286;
  ...
  const downloadTasks = pages.map((page, pageIndex) => {
    const lastPage = pageIndex === pages.length - 1;
    const summaryHeight = lastPage && summaryRows ? summaryRows * 36 + (footerMessage ? 70 : 30) : 0;
    ...
    ctx.fillRect(margin + 180, 22, 2, 116);
    ctx.fillRect(width - margin - 278, 22, 2, 116);
    if (exportLogo) {
      const logoBoxWidth = 164, logoBoxHeight = 104;
      const ratio = exportLogo.naturalWidth / exportLogo.naturalHeight;
      const drawWidth = Math.min(logoBoxWidth, logoBoxHeight * ratio);
      const drawHeight = drawWidth / ratio;
      ctx.drawImage(exportLogo, margin + 8 + (logoBoxWidth - drawWidth) / 2, 28 + (logoBoxHeight - drawHeight) / 2, drawWidth, drawHeight);
    }
    else {
      ctx.strokeRect(margin + 44, 34, 88, 88);
      ctx.font = "bold 30px Georgia";
      ctx.textAlign = "center";
      ctx.fillText(initials, margin + 88, 60);
    }
    const companyCenter = (margin + 182 + width - margin - 278) / 2;
    ...
    if (meta.length) {
      ...
      meta.forEach(([label, value], index) => {
        ...
        const featured = index === featuredMetaIndex;
        if (featured) {
          ctx.lineWidth = 2;
          ctx.strokeRect(metaX, 238, metaWidth - 12, 67);
          ctx.lineWidth = 1;
        }
        ctx.font = "bold 11px Arial";
        ctx.fillText(clean(label).toUpperCase(), metaX + (featured ? 10 : 0), featured ? 248 : 247, metaWidth - (featured ? 32 : 14));
        ctx.font = featured ? "bold 23px Georgia" : "14px Arial";
        const valueLines = wrap(ctx, clean(value) || "—", metaWidth - (featured ? 32 : 14));
        ctx.fillText(valueLines[0], metaX + (featured ? 10 : 0), featured ? 268 : 265, metaWidth - (featured ? 32 : 14));
      });
    }
    ...
    return downloadCanvas(canvas, `${base}${pageSuffix}.png`, pageIndex * 250);
  });
