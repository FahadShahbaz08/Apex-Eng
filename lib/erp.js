...
function saveSale(state, values, preserved = {}) {
  ...
  const saleLines = values.lines.map(line => {
    ...
    const legacyOverallLabour = Number(line.quantity) * Number(line.labourCost || 0), legacyOverallLaser = Number(line.quantity) * Number(line.laserMarkingCost || 0);
    return { ...line, quantity: Number(line.quantity), rate: Number(line.rate), labourCost: isRawMaterial ? 0 : Number(line.labourCost ?? item?.labourCost ?? 0), laserMarkingCost: isRawMaterial ? 0 : Number(line.laserMarkingCost ?? item?.laserMarkingCost ?? 0), labourCharge: isRawMaterial ? Number(line.labourCharge ?? item?.defaultLabourCharge ?? legacyOverallLabour) : 0, laserCharge: isRawMaterial ? Number(line.laserCharge ?? legacyOverallLaser) : 0, labourSupplierId: line.labourSupplierId || values.labourSupplierId || item?.labourSupplierId || defaultLabourSupplier?.id || "", itemNameSnapshot: line.itemNameSnapshot || item?.name || "Item" };
  });
  const chargeGroups = new Map();
  saleLines.forEach(line => {
    const overallLabourCharge = Number(line.labourCharge || 0), overallLaserCharge = Number(line.laserCharge || 0), labourTotal = overallLabourCharge + Number(line.quantity) * Number(line.labourCost || 0), laserTotal = overallLaserCharge + Number(line.quantity) * Number(line.laserMarkingCost || 0), chargeTotal = labourTotal + laserTotal;
    ...
    if (overallLaserCharge) group.detailLines.push({ itemId: line.itemId, itemName: `${line.itemNameSnapshot} — Overall laser charges`, quantity: 1, rate: overallLaserCharge, lineTotal: overallLaserCharge });
