...
export function postPayment(state, values) {
  ...
  state.payments.unshift({ id: id("PMT"), document, ...values, paymentMethod, amount, chequeStatus: paymentMethod === "Cheque" ? "Pending" : "Not applicable", posted: true });
  ...
  audit(state, `Posted ${values.direction.toLowerCase()} ${document}`, `${paymentMethod}${values.chequeDate ? ` · cheque date ${values.chequeDate}` : ""}`);
}
