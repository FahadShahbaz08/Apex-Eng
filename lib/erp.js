...
function saveSale(state, values, preserved = {}) {
  ...
  const saleLines = values.lines.map(line => {
    ...
    return { ...line, quantity: Number(line.quantity), rate: Number(line.rate), labourCost: Number(line.labourCost ?? item?.labourCost ?? 0), laserMarkingCost: Number(line.laserMarkingCost ?? item?.laserMarkingCost ?? 0), labourSupplierId: line.labourSupplierId || item?.labourSupplierId || defaultLabourSupplier?.id || "", itemNameSnapshot: line.itemNameSnapshot || item?.name || "Item" };
  });
  ...
  saleLines.forEach(line => {
    const item = state.items.find(i => i.id === line.itemId);
    inventoryCogs += Number(line.quantity) * Number(item.cost || 0);
  ...
  const lines = [{ accountName: "Sales", debit: 0, credit: total }, { accountName: "Cost of Goods Sold", debit: inventoryCogs, credit: 0 }, { accountName: "Finished Goods Inventory", debit: 0, credit: inventoryCogs }];
