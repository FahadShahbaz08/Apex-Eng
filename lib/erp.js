...
export function addItem(state, values) {
  ...
  const itemId = id("ITM");
  state.items.push({ id: itemId, ...values, sku, cost: Number(values.cost || 0), saleRate: Number(values.saleRate || 0), minStock: Number(values.minStock || 0), labourCost: Number(values.labourCost || 0), laserMarkingCost: Number(values.laserMarkingCost || 0), labourSupplierId: values.labourSupplierId || "" });
...
export function updateItem(state, itemId, values) {
  ...
  Object.assign(item, values, { id: item.id, sku: String(values.sku || item.sku).trim(), type: nextType, cost: Number(values.cost || 0), saleRate: Number(values.saleRate || 0), minStock: Number(values.minStock || 0), labourCost: Number(values.labourCost ?? item.labourCost ?? 0), laserMarkingCost: Number(values.laserMarkingCost ?? item.laserMarkingCost ?? 0), labourSupplierId: values.labourSupplierId ?? item.labourSupplierId ?? "" });
...
function saveSale(state, values, preserved = {}) {
  ...
  const saleLines = values.lines.map(line => {
    ...
    return { ...line, quantity: Number(line.quantity), rate: Number(line.rate), labourCost: isRawMaterial ? 0 : Number(line.labourCost ?? item?.labourCost ?? 0), laserMarkingCost: isRawMaterial ? 0 : Number(line.laserMarkingCost ?? item?.laserMarkingCost ?? 0), labourCharge: isRawMaterial ? Number(line.labourCharge ?? legacyOverallCharge) : 0, labourSupplierId: line.labourSupplierId || values.labourSupplierId || item?.labourSupplierId || defaultLabourSupplier?.id || "", itemNameSnapshot: line.itemNameSnapshot || item?.name || "Item" };
  });
