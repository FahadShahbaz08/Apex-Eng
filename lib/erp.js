...
function saveSale(state, values, preserved = {}) {
  ...
  const saleLines = values.lines.map(line => {
    ...
    return { ...line, quantity: Number(line.quantity), rate: Number(line.rate), labourCost: Number(line.labourCost ?? item?.labourCost ?? 0), laserMarkingCost: Number(line.laserMarkingCost ?? item?.laserMarkingCost ?? 0), labourSupplierId: values.labourSupplierId || line.labourSupplierId || item?.labourSupplierId || defaultLabourSupplier?.id || "", itemNameSnapshot: line.itemNameSnapshot || item?.name || "Item" };
  });
