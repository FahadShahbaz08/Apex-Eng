...
function saveSale(state, values, preserved = {}) {
  ...
  const saleLines = values.lines.map(line => {
    ...
    return { ...line, quantity: Number(line.quantity), rate: Number(line.rate), labourCost: Number(line.labourCost ?? (isRawMaterial ? 0 : item?.labourCost) ?? 0), laserMarkingCost: Number(line.laserMarkingCost ?? (isRawMaterial ? 0 : item?.laserMarkingCost) ?? 0), labourSupplierId: values.labourSupplierId || line.labourSupplierId || item?.labourSupplierId || defaultLabourSupplier?.id || "", itemNameSnapshot: line.itemNameSnapshot || item?.name || "Item" };
  });
  const chargeGroups = new Map();
  saleLines.forEach(line => {
    const labourTotal = Number(line.quantity) * Number(line.labourCost || 0), laserTotal = Number(line.quantity) * Number(line.laserMarkingCost || 0), chargeTotal = labourTotal + laserTotal;
    ...
    if (line.labourCost) group.detailLines.push({ itemId: line.itemId, itemName: `${line.itemNameSnapshot} — Labour`, quantity: line.quantity, rate: line.labourCost, lineTotal: labourTotal });
