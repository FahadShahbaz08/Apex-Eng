import { dateNow, nextDocumentNo, id } from "./schema.js";
...
const postJournal = (state, document, description, lines, partyId = null, date = dateNow(), metadata = {}) => {
  const debit = lines.reduce((s, l) => s + Number(l.debit || 0), 0);
  const credit = lines.reduce((s, l) => s + Number(l.credit || 0), 0);
  if (Math.abs(debit - credit) > 0.01) throw new Error(`Journal is not balanced for ${document}.`);
  state.journal.unshift({ id: id("JRN"), date, document, description, partyId, lines, ...metadata });
};

const postReceivedChequeSettlement = (state, payment, account, date = dateNow()) => {
  if (state.journal.some(entry => entry.chequeSettlementFor === payment.id)) return;
  const party = state.parties.find(row => row.id === payment.partyId);
  postJournal(state, payment.document, `Cheque cleared - ${party?.name || "Customer"}`, [
    { accountId: account.id, accountName: account.name, debit: Number(payment.amount), credit: 0 },
    { accountName: "Cheques in Hand", debit: 0, credit: Number(payment.amount) },
  ], payment.partyId, date, { chequeSettlementFor: payment.id });
};
...
export function addItem(state, values) {
  ...
  const itemId = id("ITM");
  state.items.push({ id: itemId, ...values, sku, cost: Number(values.cost || 0), saleRate: Number(values.saleRate || 0), minStock: Number(values.minStock || 0), labourCost: Number(values.labourCost || 0), laserMarkingCost: Number(values.laserMarkingCost || 0), defaultLabourCharge: values.type === "Raw Material" ? 0 : Number(values.defaultLabourCharge || 0), labourSupplierId: values.labourSupplierId || "" });
...
export function updateItem(state, itemId, values) {
  ...
  Object.assign(item, values, { id: item.id, sku: String(values.sku || item.sku).trim(), type: nextType, cost: Number(values.cost || 0), saleRate: Number(values.saleRate || 0), minStock: Number(values.minStock || 0), labourCost: Number(values.labourCost ?? item.labourCost ?? item.defaultLabourCharge ?? 0), laserMarkingCost: Number(values.laserMarkingCost ?? item.laserMarkingCost ?? 0), defaultLabourCharge: nextType === "Raw Material" ? 0 : Number(values.defaultLabourCharge ?? item.defaultLabourCharge ?? 0), labourSupplierId: values.labourSupplierId ?? item.labourSupplierId ?? "" });
...
export function deleteParty(state, partyId) {
  const party = state.parties.find(row => row.id === partyId);
  if (!party) throw new Error("Customer or supplier no longer exists.");
  const used = state.sales.some(row => row.partyId === partyId) || state.purchases.some(row => row.partyId === partyId) || state.servicePurchases.some(row => row.partyId === partyId) || state.payments.some(row => row.partyId === partyId) || state.pendingOrders.some(row => row.partyId === partyId) || state.partyLedger.some(row => row.partyId === partyId && row.document !== "OPENING") || state.journal.some(row => row.partyId === partyId);
...
function savePurchase(state, values, preserved = {}) {
  ...
  if (!values.lines?.length || values.lines.some(line => !state.items.some(item => item.id === line.itemId) || !(Number(line.quantity) > 0) || Number(line.rate) < 0)) throw new Error("Every purchase line needs a valid item, quantity and purchase rate.");
  const document = preserved.document || nextDocumentNo(state, "purchase", state.settings.purchasePrefix, state.purchases);
  const supplierInvoice = String(values.supplierInvoice || "").trim();
  if (!supplierInvoice) throw new Error("Enter the supplier invoice number.");
  if (state.purchases.some(p => p.id !== preserved.id && p.partyId === values.partyId && String(p.supplierInvoice || "").trim().toLowerCase() === supplierInvoice.toLowerCase())) throw new Error("This supplier invoice number has already been posted for the selected vendor.");
  const purchaseLines = values.lines.map(line => ({ ...line, quantity: Number(line.quantity), rate: Number(line.rate), rackId: line.rackId || "" }));
  const total = purchaseLines.reduce((sum, line) => sum + line.quantity * line.rate, 0) + Number(values.charges || 0) - Number(values.discount || 0);
  ...
  const itemCostsBefore = {};
  purchaseLines.forEach(line => {
    const item = state.items.find(i => i.id === line.itemId);
    if (!(line.itemId in itemCostsBefore)) itemCostsBefore[line.itemId] = Number(item.cost || 0);
    const previousQty = stockFor(state, line.itemId);
    const oldValue = previousQty * Number(item.cost || 0);
    const lineQty = line.quantity;
    const lineValue = lineQty * line.rate;
  ...
  state.purchases.unshift({ id: preserved.id || id("PUR"), document, ...values, supplierInvoice, lines: purchaseLines, itemCostsBefore: preserved.itemCostsBefore || itemCostsBefore, total, paid, status: paid >= total ? "Paid" : paid > 0 ? "Partially Paid" : "Unpaid", posted: true, updatedAt: preserved.id ? new Date().toISOString() : undefined });
  ...
  audit(state, `${preserved.id ? "Updated" : "Posted"} purchase ${document}`);
  return document;
}

export function postPurchase(state, values) { return savePurchase(state, values); }

export function updatePurchase(state, purchaseId, values) {
  const existing = state.purchases.find(row => row.id === purchaseId);
  if (!existing) throw new Error("Supplier invoice no longer exists.");
  state.purchases = state.purchases.filter(row => row.id !== purchaseId);
  state.stockMovements = state.stockMovements.filter(row => row.document !== existing.document);
  state.partyLedger = state.partyLedger.filter(row => row.document !== existing.document);
  state.journal = state.journal.filter(row => row.document !== existing.document);
  return savePurchase(state, values, { id: existing.id, document: existing.document, itemCostsBefore: existing.itemCostsBefore });
}

function saveServicePurchase(state, values, preserved = {}) {
  const supplier = state.parties.find(p => p.id === values.partyId && ["Supplier", "Both"].includes(p.type));
  if (!supplier) throw new Error("Select a valid service supplier.");
  if (!values.lines?.length || values.lines.some(line => !String(line.service || "").trim() || !(Number(line.quantity) > 0) || Number(line.rate) < 0)) throw new Error("Every service line needs a service name, quantity and valid rate.");
  const lines = values.lines.map(line => ({ service: String(line.service).trim(), description: String(line.description || "").trim(), quantity: Number(line.quantity), rate: Number(line.rate), lineTotal: Number(line.quantity) * Number(line.rate) }));
  const total = lines.reduce((sum, line) => sum + line.lineTotal, 0), paid = Number(values.paid || 0);
  if (!(total > 0)) throw new Error("Service bill total must be greater than zero.");
  if (paid > total) throw new Error("Paid amount cannot exceed service bill total.");
  if (paid > 0 && !state.accounts.some(account => account.id === values.accountId)) throw new Error("Select the cash or bank account used for the service payment.");
  const document = preserved.document || nextDocumentNo(state, "servicePurchase", "SRV", state.servicePurchases);
  state.servicePurchases.unshift({ id: preserved.id || id("SRV"), document, ...values, lines, total, paid, status: paid >= total ? "Paid" : paid > 0 ? "Partially Paid" : "Unpaid", posted: true, updatedAt: preserved.id ? new Date().toISOString() : undefined });
  const detailLines = lines.map(line => ({ itemName: `${line.service}${line.description ? ` — ${line.description}` : ""}`, quantity: line.quantity, rate: line.rate, lineTotal: line.lineTotal }));
  state.partyLedger.unshift({ id: id("PLD"), date: values.date, partyId: supplier.id, document, description: "Outside service bill", debit: 0, credit: total, detailLines });
  if (paid) state.partyLedger.unshift({ id: id("PLD"), date: values.date, partyId: supplier.id, document, description: "Payment with service bill", debit: paid, credit: 0 });
  const journalLines = lines.map(line => ({ accountName: `Outside Service - ${line.service}`, debit: line.lineTotal, credit: 0 }));
  if (paid) journalLines.push({ accountId: values.accountId, accountName: state.accounts.find(account => account.id === values.accountId)?.name || "Cash/Bank", debit: 0, credit: paid });
  if (total - paid) journalLines.push({ accountName: "Accounts Payable", debit: 0, credit: total - paid });
  postJournal(state, document, `Outside services - ${supplier.name}`, journalLines, supplier.id, values.date);
  audit(state, `${preserved.id ? "Updated" : "Posted"} service bill ${document}`, `${supplier.name} · ${money(total)}`);
  return document;
}

export function postServicePurchase(state, values) { return saveServicePurchase(state, values); }

export function updateServicePurchase(state, servicePurchaseId, values) {
  const existing = state.servicePurchases.find(row => row.id === servicePurchaseId);
  if (!existing) throw new Error("Service bill no longer exists.");
  state.servicePurchases = state.servicePurchases.filter(row => row.id !== servicePurchaseId);
  state.partyLedger = state.partyLedger.filter(row => row.document !== existing.document);
  state.journal = state.journal.filter(row => row.document !== existing.document);
  return saveServicePurchase(state, values, { id: existing.id, document: existing.document });
}

export function deleteServicePurchase(state, servicePurchaseId) {
  const existing = state.servicePurchases.find(row => row.id === servicePurchaseId);
  if (!existing) throw new Error("Service bill no longer exists.");
  const supplier = state.parties.find(row => row.id === existing.partyId);
  state.servicePurchases = state.servicePurchases.filter(row => row.id !== servicePurchaseId);
  state.partyLedger = state.partyLedger.filter(row => row.document !== existing.document);
  state.journal = state.journal.filter(row => row.document !== existing.document);
  audit(state, `Deleted service bill ${existing.document}`, `${supplier?.name || "Unknown supplier"} · ${money(existing.total)}`);
...
function saveSale(state, values, preserved = {}) {
  ...
  if (!values.lines?.length || values.lines.some(line => !state.items.some(item => item.id === line.itemId) || !(Number(line.quantity) > 0) || Number(line.rate) < 0)) throw new Error("Every sales line needs a valid item, quantity and sale rate.");
  ...
  const defaultLabourSupplier = state.parties.find(p => ["Supplier", "Both"].includes(p.type) && p.name.trim().toLowerCase() === "abu ki mazduri");
  const saleLines = values.lines.map(line => {
    const item = state.items.find(row => row.id === line.itemId);
    const isRawMaterial = item?.type === "Raw Material";
    const quantity = Number(line.quantity);
    const rawLabourRate = Number(line.labourCharge || 0) > 0 && quantity > 0 ? Number(line.labourCharge) / quantity : Number(line.labourCost ?? item?.labourCost ?? item?.defaultLabourCharge ?? 0);
    const rawLaserRate = Number(line.laserCharge || 0) > 0 && quantity > 0 ? Number(line.laserCharge) / quantity : Number(line.laserMarkingCost || 0);
    return { ...line, quantity, rate: Number(line.rate), labourCost: isRawMaterial ? rawLabourRate : Number(line.labourCost ?? item?.labourCost ?? 0), laserMarkingCost: isRawMaterial ? rawLaserRate : Number(line.laserMarkingCost ?? item?.laserMarkingCost ?? 0), labourCharge: 0, laserCharge: 0, labourSupplierId: line.labourSupplierId || values.labourSupplierId || item?.labourSupplierId || defaultLabourSupplier?.id || "", itemNameSnapshot: line.itemNameSnapshot || item?.name || "Item" };
  });
  const chargeGroups = new Map();
  saleLines.forEach(line => {
    const overallLabourCharge = Number(line.labourCharge || 0), labourTotal = overallLabourCharge + Number(line.quantity) * Number(line.labourCost || 0), laserTotal = Number(line.quantity) * Number(line.laserMarkingCost || 0), chargeTotal = labourTotal + laserTotal;
    if (!chargeTotal) return;
    const supplier = state.parties.find(p => p.id === line.labourSupplierId && ["Supplier", "Both"].includes(p.type));
    if (!supplier) throw new Error(`Select a labour supplier for ${line.itemNameSnapshot}. Create or select the supplier "Abu Ki Mazduri".`);
    const group = chargeGroups.get(supplier.id) || { supplier, labourTotal: 0, laserTotal: 0, detailLines: [] };
    group.labourTotal += labourTotal; group.laserTotal += laserTotal;
    if (overallLabourCharge) group.detailLines.push({ itemId: line.itemId, itemName: `${line.itemNameSnapshot} — Overall labour charges`, quantity: 1, rate: overallLabourCharge, lineTotal: overallLabourCharge });
    if (line.labourCost) group.detailLines.push({ itemId: line.itemId, itemName: `${line.itemNameSnapshot} — Labour`, quantity: line.quantity, rate: line.labourCost, lineTotal: Number(line.quantity) * Number(line.labourCost) });
    if (line.laserMarkingCost) group.detailLines.push({ itemId: line.itemId, itemName: `${line.itemNameSnapshot} — Laser marking`, quantity: line.quantity, rate: line.laserMarkingCost, lineTotal: laserTotal });
    chargeGroups.set(supplier.id, group);
  });
  const document = preserved.document || nextDocumentNo(state, "invoice", state.settings.invoicePrefix, state.sales);
  const subtotal = saleLines.reduce((sum, line) => sum + Number(line.quantity) * Number(line.rate), 0);
  ...
  let inventoryCogs = 0, rawMaterialCogs = 0, finishedGoodsCogs = 0;
  saleLines.forEach(line => {
    const item = state.items.find(i => i.id === line.itemId);
    const lineCogs = Number(line.quantity) * Number(item.cost || 0);
    inventoryCogs += lineCogs;
    if (item.type === "Raw Material") rawMaterialCogs += lineCogs;
    else finishedGoodsCogs += lineCogs;
  ...
  const labourPayables = [...chargeGroups.values()].map(group => ({ supplierId: group.supplier.id, supplierName: group.supplier.name, labourTotal: group.labourTotal, laserTotal: group.laserTotal, total: group.labourTotal + group.laserTotal, detailLines: group.detailLines }));
  const cogs = inventoryCogs + labourPayables.reduce((sum, payable) => sum + payable.total, 0);
  state.sales.unshift({ id: preserved.id || id("SAL"), document, ...values, lines: saleLines, labourPayables, subtotal, total, paid, cogs, status: paid >= total ? "Paid" : paid > 0 ? "Partially Paid" : "Unpaid", posted: true, updatedAt: preserved.id ? new Date().toISOString() : undefined });
  ...
  const lines = [{ accountName: "Sales", debit: 0, credit: total }, { accountName: "Cost of Goods Sold", debit: inventoryCogs, credit: 0 }];
  if (finishedGoodsCogs) lines.push({ accountName: "Finished Goods Inventory", debit: 0, credit: finishedGoodsCogs });
  if (rawMaterialCogs) lines.push({ accountName: "Raw Material Inventory", debit: 0, credit: rawMaterialCogs });
  ...
  labourPayables.forEach(payable => {
    state.partyLedger.unshift({ id: id("PLD"), date: values.date, partyId: payable.supplierId, document, description: `Product labour and laser marking for ${document}`, debit: 0, credit: payable.total, detailLines: payable.detailLines });
    const payableLines = [];
    if (payable.labourTotal) payableLines.push({ accountName: "Product Labour Cost", debit: payable.labourTotal, credit: 0 });
    if (payable.laserTotal) payableLines.push({ accountName: "Laser Marking Cost", debit: payable.laserTotal, credit: 0 });
    payableLines.push({ accountName: "Accounts Payable", debit: 0, credit: payable.total });
    postJournal(state, document, `Labour payable - ${payable.supplierName}`, payableLines, payable.supplierId, values.date);
  });
...
export function addPendingOrder(state, values) {
  ...
  const order = { id: id("ORD"), document: nextDocumentNo(state, "pendingOrder", "ORD", state.pendingOrders), partyId: values.partyId, pendingDate: values.pendingDate || dateNow(), requiredDate: values.requiredDate || "", notes: values.notes || "", lines: values.lines.map(l => ({ itemId: l.itemId, quantity: Number(l.quantity), rate: Number(l.rate) })), status: "Pending", createdAt: new Date().toISOString() };
  state.pendingOrders.unshift(order); audit(state, `Created pending order ${order.document}`); return order.document;
}

export function updatePendingOrder(state, orderId, values) {
  const order = state.pendingOrders.find(row => row.id === orderId);
  if (!order) throw new Error("Pending order no longer exists.");
  const customer = state.parties.find(p => p.id === values.partyId && ["Customer", "Both"].includes(p.type));
  if (!customer) throw new Error("Select a valid customer.");
  if (!values.lines?.length || values.lines.some(l => !l.itemId || Number(l.quantity) <= 0 || Number(l.rate) < 0)) throw new Error("Every pending-order line needs an item, quantity and valid price.");
  Object.assign(order, { partyId: values.partyId, pendingDate: values.pendingDate || order.pendingDate, requiredDate: values.requiredDate || "", notes: values.notes || "", lines: values.lines.map(l => ({ itemId: l.itemId, quantity: Number(l.quantity), rate: Number(l.rate) })), updatedAt: new Date().toISOString() });
  audit(state, `Updated pending order ${order.document}`);
...
export function postProduction(state, values) {
  ...
  const quantity = Number(values.quantity), outputQty = Number(bom.outputQty);
  if (!(quantity > 0)) throw new Error("Production quantity must be greater than zero.");
  if (!(outputQty > 0)) throw new Error(`BOM "${bom.name}" has an invalid standard output quantity. Edit the BOM and enter a value greater than zero.`);
  const product = state.items.find(i => i.id === bom.productId);
  if (!product) throw new Error(`BOM "${bom.name}" is linked to a product that no longer exists. Edit the BOM and select its finished product again.`);
  if (!Array.isArray(bom.lines) || !bom.lines.length) throw new Error(`BOM "${bom.name}" has no raw materials. Edit the BOM before posting production.`);
  const issueRacks = values.issueRacks || {};
  const missingMaterial = bom.lines.find(line => !state.items.some(item => item.id === line.itemId));
  if (missingMaterial) throw new Error(`BOM "${bom.name}" contains a raw material that no longer exists. Edit the BOM and replace the missing material.`);
  const multiplier = quantity / outputQty;
  for (const line of bom.lines) {
    const required = Number(line.quantity) * multiplier;
    if (!(required > 0)) throw new Error(`BOM material "${state.items.find(i => i.id === line.itemId)?.name}" has an invalid quantity. Edit the BOM and enter a value greater than zero.`);
    const rackId = issueRacks[line.itemId] || line.rackId;
  ...
  const document = nextDocumentNo(state, "production", state.settings.productionPrefix, state.productions);
  let materialCost = 0;
  bom.lines.forEach(line => {
    const item = state.items.find(i => i.id === line.itemId);
    const required = Number(line.quantity) * multiplier;
    materialCost += required * Number(item.cost || 0);
    addStock(state, { itemId: line.itemId, rackId: issueRacks[line.itemId] || line.rackId, quantity: -required, rate: item.cost, type: "Production consumption", document, date: values.date });
  });
  ...
  product.cost = oldQty + quantity > 0 ? (oldQty * Number(product.cost || 0) + totalCost) / (oldQty + quantity) : totalCost / quantity;
  addStock(state, { itemId: product.id, rackId: values.outputRackId, quantity, rate: totalCost / quantity, type: "Production output", document, date: values.date });
  state.productions.unshift({ id: id("PRD"), document, ...values, issueRacks, quantity, bomSnapshot: structuredClone(bom), materialCost, extraCosts, conversionCost, totalCost, unitCost: totalCost / quantity, status: "Posted" });
...
function savePayment(state, values, existing = null) {
  ...
  if (isReceipt && paymentMethod === "Cheque") {
    postJournal(state, document, `${values.direction} - ${party.name}`, [{ accountName: "Cheques in Hand", debit: amount, credit: 0 }, { accountName: "Accounts Receivable", debit: 0, credit: amount }], party.id, values.date);
    if (chequeStatus === "Cleared") {
      const savedPayment = state.payments.find(row => row.document === document);
      postReceivedChequeSettlement(state, savedPayment, account, existing?.chequeStatusUpdatedAt?.slice(0, 10) || dateNow());
    }
  } else if (isReceipt) {
...
export function updateChequeStatus(state, paymentId, status, accountId = "") {
  ...
  if (payment.direction === "Receipt") {
    const receivingAccount = state.accounts.find(row => row.id === (accountId || payment.accountId));
    if (status === "Cleared" && !receivingAccount) throw new Error("Select the cash or bank account receiving this cheque.");

    // Older versions posted received cheques straight to the selected account.
    // Convert that legacy entry into Cheques in Hand before applying the clearing entry.
    state.journal
      .filter(entry => entry.document === payment.document && !entry.chequeSettlementFor && entry.description?.startsWith("Receipt -"))
      .forEach(entry => {
        entry.lines = entry.lines.map(line => line.accountId
          ? { accountName: "Cheques in Hand", debit: line.debit, credit: line.credit }
          : line);
      });

    state.journal = state.journal.filter(entry => entry.chequeSettlementFor !== payment.id);
    if (status === "Cleared") {
      payment.accountId = receivingAccount.id;
      postReceivedChequeSettlement(state, payment, receivingAccount, dateNow());
    }
  }
...
export function addExpenseCategory(state, values) {
  const name = String(values.name || "").trim();
  if (!name) throw new Error("Enter an expense category name.");
  if (state.expenseCategories.some(category => category.name.trim().toLowerCase() === name.toLowerCase())) throw new Error("This expense category already exists.");
  const category = { id: id("EXC"), name, description: String(values.description || "").trim(), createdAt: new Date().toISOString() };
  state.expenseCategories.push(category);
  audit(state, `Created expense category ${name}`);
  return category.id;
}

export function updateExpenseCategory(state, categoryId, values) {
  const category = state.expenseCategories.find(row => row.id === categoryId);
  if (!category) throw new Error("Expense category no longer exists.");
  const name = String(values.name || "").trim();
  if (!name) throw new Error("Enter an expense category name.");
  if (state.expenseCategories.some(row => row.id !== categoryId && row.name.trim().toLowerCase() === name.toLowerCase())) throw new Error("This expense category already exists.");
  const previousName = category.name;
  Object.assign(category, { name, description: String(values.description || "").trim(), updatedAt: new Date().toISOString() });
  state.expenses.filter(expense => expense.categoryId === categoryId).forEach(expense => { expense.category = name; });
  state.journal.forEach(entry => entry.lines.forEach(line => { if (line.accountName === `Expense - ${previousName}`) line.accountName = `Expense - ${name}`; }));
  audit(state, `Updated expense category ${previousName}`, name);
  return category.id;
}

export function deleteExpenseCategory(state, categoryId) {
  const category = state.expenseCategories.find(row => row.id === categoryId);
  if (!category) throw new Error("Expense category no longer exists.");
  const used = state.expenses.some(expense => expense.categoryId === categoryId || (!expense.categoryId && String(expense.category || "").trim().toLowerCase() === category.name.trim().toLowerCase()));
  if (used) throw new Error("This category has expense history and cannot be deleted. Rename it instead to preserve accounting records.");
  state.expenseCategories = state.expenseCategories.filter(row => row.id !== categoryId);
  audit(state, `Deleted expense category ${category.name}`);
}

export function postExpense(state, values) {
  const account = state.accounts.find(a => a.id === values.accountId);
  if (!account) throw new Error("Select a cash/bank account.");
  const category = state.expenseCategories.find(row => row.id === values.categoryId);
  if (!category) throw new Error("Create or select a valid expense category.");
  const document = id("EXP");
  const amount = Number(values.amount);
  if (!(amount > 0)) throw new Error("Expense amount must be greater than zero.");
  state.expenses.unshift({ id: id("EXR"), document, ...values, categoryId: category.id, category: category.name, amount, status: "Posted" });
  postJournal(state, document, values.description, [{ accountName: `Expense - ${category.name}`, debit: amount, credit: 0 }, { accountId: account.id, accountName: account.name, debit: 0, credit: amount }], null, values.date);
